...
export const useCart = () => {//Exportamos hook personalizado
  ...
  //Asignamos el tipo guitarra al parametro de la siguiente funcion para decirle a typeScript que es un objeto
  const addCart = (item : Guitar) => {
    ...
    if (itemExist >= 0) {
      ...
      setCart([...cart, newItem]); //actualizamos el carrito con el nuevo producto
    }
  };

  //Asignamos el tipo del id del objeto de guitarra como tipo number o string con el lookup de typeScript
